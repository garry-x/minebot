 ...
 *
 * @param {string} pidFile  Absolute path to the PID lock file.
 * @param {object} [options]
 * @param {number} [options.pid]  PID to write (defaults to process.pid).
 * @param {Function} [options.onStale]  Called with (stalePid) when a stale lock is removed.
 * @throws {Error} If another instance is already running.
 */
function acquireLock(pidFile, options = {}) {
 ...
 * @param {string} pidFile
 * @returns {boolean} true if the lock is held by a live process.
 */
function isLocked(pidFile) {
