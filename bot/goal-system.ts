...
class GoalSystem {
  static resourceCategories = resourceCategories;

  static getResourceCategory(itemName: string): string | null {
    const categories = this.resourceCategories as unknown as Record<string, string[]>;
  ...
  static getAllItemsInCategory(category: string): string[] {
    const cat = this.resourceCategories[category as ResourceCategory];
    return cat ? [...cat] : [];
  }

  static countItemsByCategory(inventory: InventoryItem[]): CategoryCount {
  ...
  static goals: Record<string, Goal> = {
    basic_survival: {
      id: 'basic_survival',
      name: '基础生存',
  ...
  static getGoal(goalId: string): Goal {
  ...
  static createGoalState(goalId: string, botId: string): GoalStateData {
  ...
  static calculateProgress(goalState: GoalStateData, inventory: InventoryItem[]): ProgressResult {
    ...
    for (const task of goalState.subTasks) {
      if (task.completed && !task.optional) {
        completedTasks++;
      } else if (task.targetCategory && !task.optional && typeof task.required === 'number') {
      ...
      } else if (task.target && task.type !== 'build' && !task.optional && typeof task.required === 'number') {
  ...
  static updateGoalProgress(goalState: GoalStateData, inventory: InventoryItem[] | null | undefined): GoalStateData {
    ...
    for (const task of goalState.subTasks) {
      ...
      if (task.targetCategory && !task.optional && typeof task.required === 'number') {
        const count = categoryCount[task.targetCategory] || 0;
        task.progress = Math.min(1, count / task.required);
        if (task.progress >= 1) {
          task.completed = true;
